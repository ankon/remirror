...
export interface Suggester<Schema extends EditorSchema = EditorSchema> {
  ...
  isValidPosition?: (
    ...
    match: SuggestStateMatch<Schema>,
  ) => boolean;
}
 ...
 * - `to` - describes the end position of the match, so the point at which
 *   there is no longer an active suggest.
 ...
 * preserving the remaining part of the match, then you can use `from`, `cursor` for
 * the initial replacement and then take the value between `cursor` and `to` and
 * append it in whatever way you feel works.
 */
 ...
 *
 */
export interface SuggestStateMatch<Schema extends EditorSchema = EditorSchema>
...
export interface SuggestCallbackParameter<Schema extends EditorSchema = EditorSchema>
  extends SuggestStateMatch<Schema>,
...
export interface SuggestChangeHandlerParameter<Schema extends EditorSchema = EditorSchema>
  extends SuggestCallbackParameter<Schema>,
    ReasonParameter {}
...
/**
 * The reason for the suggest state.
 */
export interface SuggestStateMatchReason<Schema extends EditorSchema = EditorSchema>
  extends SuggestStateMatch<Schema>,
    ReasonParameter {}
...
export interface SuggestReasonMap<Schema extends EditorSchema = EditorSchema> {
   ...
   */
  change?: SuggestStateMatchReason<Schema>;

  /**
   * Exit reasons for triggering the change handler.
   */
  exit?: SuggestStateMatchReason<Schema>;
}
...
export interface CompareMatchParameter<Schema extends EditorSchema = EditorSchema> {
   ...
   * The initial match
   */
  prev: SuggestStateMatch<Schema>;

  /**
   * The current match
   */
  next: SuggestStateMatch<Schema>;
}
