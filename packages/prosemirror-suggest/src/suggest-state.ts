...
export class SuggestState<Schema extends EditorSchema = EditorSchema> {
   ...
   */
  #next?: Readonly<SuggestStateMatch<Schema>>;

  /**
   * Holds onto the previous active state.
   */
  #prev?: Readonly<SuggestStateMatch<Schema>>;
   ...
   */
  get match(): Readonly<SuggestStateMatch<Schema>> | undefined {
    return this.#next
      ? this.#next
   ...
   */
  private createParameter(match: SuggestStateMatch<Schema>): SuggestCallbackParameter<Schema> {
    return {
      view: this.view,
  ...
  /**
   * Create the prop to be passed into the `onChange` handler.
   */
  private createReasonParameter(
    match: SuggestStateMatchReason<Schema>,
  ): SuggestChangeHandlerParameter<Schema> {
    return {
  ...
  private onViewUpdate() {
    ...
    if (exit && this.shouldRunExit()) {
      exit.suggester.onChange(this.createReasonParameter(exit));
      this.#removed = false;
  ...
  toJSON(): SuggestStateMatch<Schema> | undefined {
    return this.match;
  }
